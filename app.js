...
let timbreActual = null; // {timbre, qrPngDataUrl, qrUrl, cadenaOriginal}
...
function initFacturacion() {
  ...
  document.getElementById("btn-firmar-timbrar")
    .addEventListener("click", firmarYTimbrar);
...
async function firmarYTimbrar() {
  const xmlArea = document.getElementById("xml-output");
  let xml = (xmlArea && xmlArea.value) ? xmlArea.value.trim() : "";
  if (!xml) {
    generarXml();
    xml = (xmlArea && xmlArea.value) ? xmlArea.value.trim() : "";
  }
  if (!xml) {
  ...
  try {
    // 1) Firmar (agrega NoCertificado/Certificado/Sello)
    const r1 = await fetch("http://127.0.0.1:8787/api/firmar", {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({xml})
    });
    const j1 = await r1.json();
    if (!r1.ok) throw new Error(j1.error || "Error firmando");

    const xmlFirmado = j1.xmlFirmado;
    xmlArea.value = xmlFirmado;

    // guardar cadena original para PDF
    timbreActual = timbreActual || {};
    timbreActual.cadenaOriginal = j1.cadenaOriginal || "";

    // 2) Timbrar (demo o real según backend/.env)
    const r2 = await fetch("http://127.0.0.1:8787/api/timbrar", {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({xml: xmlFirmado})
    });
    const j2 = await r2.json();
    if (!r2.ok) throw new Error(j2.error || "Error timbrando");

    xmlArea.value = j2.xmlTimbrado;

    timbreActual = {
      timbre: j2.timbre,
      qrUrl: j2.qrUrl,
      qrPngDataUrl: j2.qrPngDataUrl,
      cadenaOriginal: timbreActual.cadenaOriginal || ""
    };

    // refrescar vista de impresión para que ya muestre timbre/QR
    const clienteId = document.getElementById("fac-cliente").value;
    const cliente = clientes.find((c) => c.id === clienteId);
    if (cliente) {
      // reconstruir datos básicos desde la UI
      conceptosFactura = leerConceptosFactura();
      let subtotal = 0, ivaTotal = 0;
      conceptosFactura.forEach(c => { subtotal += c.importe; ivaTotal += c.iva; });
      const total = subtotal + ivaTotal;

      actualizarVistaImpresion(cliente, {
    ...
    alert("Listo: XML firmado y timbrado. Ya puedes imprimir/guardar PDF.");
  } catch (e) {
    console.error(e);
    alert("No se pudo firmar/timbrar. Revisa que el backend esté corriendo (localhost:8787) y que backend/.env tenga rutas correctas.\n\nDetalle: " + (e.message || e));
  }
}


// ========================= VISTA IMPRESIÓN =========================
function actualizarVistaImpresion(cliente, datosFactura, timbreInfo=null) {
  ...
  <div class="factura-pdf">
    ...
    <div class="factura-pdf-sellos">
      ...
      ${timbreInfo && timbreInfo.timbre ? `UUID: <strong>${escapeXml(timbreInfo.timbre.uuid||"")}</strong><br/>Fecha timbrado: ${escapeXml(timbreInfo.timbre.fechaTimbrado||"")}<br/><br/><img alt="QR CFDI" style="width:120px;height:120px;border:1px solid #ddd" src="${timbreInfo.qrPngDataUrl||""}"/><br/><small style="word-break:break-all;">${escapeXml(timbreInfo.qrUrl||"")}</small><br/><br/>Sello CFDI: <small style="word-break:break-all;">${escapeXml((timbreInfo.timbre.selloCFD||"").slice(0,90))}...</small><br/>Sello SAT: <small style="word-break:break-all;">${escapeXml((timbreInfo.timbre.selloSAT||"").slice(0,90))}...</small><br/><br/>Cadena original: <small style="word-break:break-all;">${escapeXml((timbreInfo.cadenaOriginal||"").slice(0,180))}...</small>` : `<em>Aún no timbrado.</em> (Ejecuta “Firmar y timbrar” para mostrar UUID/QR/Sellos.)` }
    </div>
  </div>`;
...
function registrarFacturaEnHistorico(cliente, datosFactura, conceptos) {
  ...
  const registro = {
    ...
    uuid: (timbreActual && timbreActual.timbre && timbreActual.timbre.uuid) ? timbreActual.timbre.uuid : "",
    timbre: (timbreActual && timbreActual.timbre) ? timbreActual.timbre : null,
    qrUrl: (timbreActual && timbreActual.qrUrl) ? timbreActual.qrUrl : "",
